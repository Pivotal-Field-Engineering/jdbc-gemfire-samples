package io.pivotal.kaiser.test.spring.gemfire.sink.custom.mvc;
...
public class ReflectionBasedObjectModelingTest {
	...
	public @ResponseBody void runTest() throws Exception {
		...
		LOG.info("Testing...");
		LOG.info("RegionName : {}", "Member");
		try {
			Set keyset = payloadAsMap.keySet();
			LOG.info("Fields from Map = {}", keyset.toString());

			Class K = Class.forName("io.pivotal.kaiser.gemfire.common.key.MemberKey");
			Object k = K.newInstance();
			LOG.debug("KeyClass : {}", K.getName());

			Class V = Class.forName("io.pivotal.kaiser.gemfire.common.model.Member");
			...
			Region region = cache.getRegion("Member");
